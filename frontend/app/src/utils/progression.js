// Calcule la progression d'une opportunite a afficher (barre + pourcentage).
//
// Deux phases :
//  - avant seuilMinimum : progression vers le seuil de validation
//  - apres seuilMinimum :
//      - plafonnee : progression vers le plafond
//      - illimitee : progression continue dans les paliers de prix
export function calculerProgression({ participantsActuels = 0, seuilMinimum, seuilMaximal, paliers = [] }) {
  ...
  const pctVersObjectif = objectifFinal > 0
  ...
  return {
    pct: pctVersObjectif,
    phase: valide ? 'illimitee' : 'validation',
    valide,
    objectifFinal,
  }
}
